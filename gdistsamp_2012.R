...
model$global <- gdistsamp(lambdaformula = ~scale_short+scale_averagewater+region+scale_int, 
                          phiformula = ~1, 
                          pformula = ~ 1,
                          data = umf, keyfun = "hazard", mixture="P",se = T, output="abund")
save(model, file="C:/Users/avanderlaar/Documents/unmarked/2012_p_models.Rdata")
list  = fitList(model)
model = modSel(list)
model
