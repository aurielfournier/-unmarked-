...
c12 <- read.csv("2012r3_cov.csv")
c12 <- c12[,c(1:3,6:15)] # taking out covariates that he does not need
d12 <- merge(abund,c12,by=c("impound"))
write.csv(d12, "abund_cov_2012.csv", row.names=F)

## 2013
abund <- read.csv("abundances_2013.csv")
c13 <- read.csv("2013r3_cov.csv")
c13 <- c13[,c(1:3,6:17)]
d13 <- merge(abund,c13,by=c("impound"))
write.csv(d13, "abund_cov_2013.csv", row.names=F)

## 2014
abund <- read.csv("abundances_2014.csv")
c14 <- read.csv("2014r3_cov.csv")
c14 <- c14[,c(1:3,7:11)]
d14 <- merge(abund,c14,by=c("impound"))
write.csv(d14, "abund_cov_2014.csv", row.names=F)
